...
if (typeof handleFormSubmit !== 'undefined') {
    ...
    window.handleFormSubmit = function(form) {
        ...
        // 验证所有字段
        inputs.forEach(input => {
            if (window.HydrergyMain && !window.HydrergyMain.validateField(input)) {
                isFormValid = false;
            }
        });
        
        if (!isFormValid) {
            if (window.HydrergyMain) {
                window.HydrergyMain.showNotification('message.error', 'error');
            }
            return;
        }
        ...
        submitBtn.textContent = getTranslation('message.sending');
        ...
        setTimeout(() => {
            ...
            // 显示成功消息
            if (window.HydrergyMain) {
                window.HydrergyMain.showNotification('message.success', 'success');
            }
        }, 2000);
    };
}